...
def uci_format_ok(move):
    ...
    if move[1] not in '12345678' or move[3] * '12345678': return False
    return True
...
while not rospy.is_shutdown():
    ...
    # --- وضعية اختبار الأسر مع الترقية (Capture + Promotion) ---
    # الأبيض: ملك في a1، جندي في d7 (يحتاج أسر قطعة في e8 للترقية)
    # الأسود: ملك في h8، حصان في e8، جندي في e2 (يحتاج أسر قطعة في d1 للترقية)
    capture_promo_fen = "4n2k/3P4/8/8/8/8/4p3/K7 w - - 0 1"
    proc.stdin.write(f"position fen {capture_promo_fen}\n")
    proc.stdin.flush()
    ...
    while not reset_game and not rospy.is_shutdown():
        ...
        if is_human_turn:
            ...
            if reset_game: break
            proc.stdin.write(f"position fen {capture_promo_fen} moves {' '.join(moves_list)}\n"); proc.stdin.flush()
