...
while not rospy.is_shutdown():
    ...
    # --- وضعية اختبار التبييت (ملوك وقلاع وجندي أمام كل قلعة) ---
    # الأبيض: ملك e1، قلاع a1 و h1، جنود a2 و h2
    # الأسود: ملك e8، قلاع a8 و h8، جنود a7 و h7
    castling_test_fen = "r3k2r/p6p/8/8/8/8/P6P/R3K2R w KQkq - 0 1"
    proc.stdin.write(f"position fen {castling_test_fen}\n")
    proc.stdin.flush()
    ...
    while not reset_game and not rospy.is_shutdown():
        ...
        if is_human_turn:
            ...
            if reset_game: break
            proc.stdin.write(f"position fen {castling_test_fen} moves {' '.join(moves_list)}\n"); proc.stdin.flush()
