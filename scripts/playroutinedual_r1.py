...
def update_orientation(color):
    ...
    target_home = "h8" if color == "black" else "a1"
...
for col_g in range(3):
    for row_g in range(8):
        gx = origin_x + row_g * square_size
        gy = _gy_start - col_g * square_size
        gz = origin_z
...
promotion_positions = {
    'q': (origin_x + 0 * square_size, _py, origin_z),
    'r': (origin_x + 1 * square_size, _py, origin_z),
    'b': (origin_x + 2 * square_size, _py, origin_z),
    'n': (origin_x + 3 * square_size, _py, origin_z),
}
...
def arc_move(move_group, sx, sy, ex, ey, pick_h, h_travel,
             arc_extra=0.2, vf=1, af=1,
             N_up=10, N_curve=40, N_down=10, eef_step=0.01):
    ...
    plan, fraction = move_group.compute_cartesian_path(
        waypoints, float(eef_step), False)

    if fraction < 0.9:
        rospy.logwarn(f"Arc path incomplete: fraction={fraction:.2f}")
        return False
...
def color_cb(msg):
    ...
    rospy.loginfo(f"Panda 1 is now playing as: {color}")
...
def move_callback(msg, args):
    ...
    try:
        ...
        status_pub.publish("panda1_ready")  # ← إشارة الانتهاء
...
def main():
    ...
    rospy.init_node('chessrobot_moveit', anonymous=False)
    ...
    rospy.loginfo("Panda1 Ready. Waiting for moves...")
    rospy.Subscriber('robot_move_cmd', String, move_callback,
                     (arm, grasp_client, move_client))
    rospy.Subscriber('/panda1/color', String, color_cb)                     
    rospy.spin()
