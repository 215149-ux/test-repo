...
# --- ناشري الألوان للروبوتات لضمان تصفير المقبرة وعكس الاتجاه ---
...
while not rospy.is_shutdown():
    ...
    # --- توزيع وإرسال الألوان فور استلام الإعدادات لضبط orientation الروبوتات وتصفير المقبرة ---
    mode_name = game_settings.get("mode", "Human vs Robot")
    if mode_name == "Robot vs Robot":
    ...
    # --- وضعية اختبار الترقية (مسار خالٍ تماماً أمام الجنود والملوك بعيدون) ---
    # الأبيض: ملك في a1 وجندي في e7 (الطريق أمام e8 خالٍ)
    # الأسود: ملك في h8 وجندي في d2 (الطريق أمام d1 خالٍ)
    test_fen = "7k/4P3/8/8/8/8/3p4/K7 w - - 0 1"
    proc.stdin.write(f"position fen {test_fen}\n")
    proc.stdin.flush()
    ...
    game_mode = '2' if mode_name == "Robot vs Robot" else '1'
    ...
    while not reset_game and not rospy.is_shutdown():
        ...
        if is_human_turn:
            ...
            if reset_game: break
            # استخدام وضعية الترقية لضمان استمرار اللعبة في نفس البيئة
            proc.stdin.write(f"position fen {test_fen} moves {' '.join(moves_list)}\n"); proc.stdin.flush()
