...
def cb_promotion_request(msg):
    """يستقبل طلب ترقية من board_tracker: {"move": "e7e8"}"""
    try:
        data = json.loads(msg.data)
        from_to = data.get('move', '')
        if len(from_to) == 4:
            print(f"♟ [PROMO REQUEST] from board_tracker: {from_to}")
            pub_gui_status.publish(f"__PROMOTION__:{from_to}")
    except Exception as e:
        print(f"promotion_request error: {e}")
    ...
    while not reset_game and not rospy.is_shutdown():
        ...
        if is_human_turn:
            ...
            while not reset_game and not rospy.is_shutdown():
                    ...
                    while not gui_move_ready.is_set() and not reset_game and not rospy.is_shutdown():
                       rospy.sleep(0.1)
                    if reset_game: break
                
                    mv = final_gui_move
                    print(f"📥 player move: {mv}")                    
                    if not uci_format_ok(mv):
                        pub_gui_status.publish("Invalid format! Use e2e4")
                        print(">>> Invalid format!")
                        gui_move_ready.clear(); continue
                    try:
